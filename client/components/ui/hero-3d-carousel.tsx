import React, { useRef, useState } from "react";
import { Canvas, useFrame } from "@react-three/fiber";
import { OrbitControls, Html } from "@react-three/drei";
import * as THREE from "three";
import { X } from "lucide-react";

export interface CardData {
  label: string;
  title: string;
  tags?: string[];
  type?: 'project' | 'achievement';
}

interface Hero3DCarouselProps {
  imageSrc: string;
  projects: CardData[];
  achievements?: CardData[];
}

const RotatingGroup = ({ 
  data, 
  radius = 3.5, 
  speed = 0.2, 
  yOffset = 0,
  cardClassName = ""
}: { 
  data: CardData[]; 
  radius?: number; 
  speed?: number;
  yOffset?: number;
  cardClassName?: string;
}) => {
  const groupRef = useRef<THREE.Group>(null);
  const [dismissedIndices, setDismissedIndices] = useState<Set<number>>(new Set());

  useFrame((state, delta) => {
    if (groupRef.current) {
      groupRef.current.rotation.y += delta * speed;
    }
  });

  const handleDismiss = (index: number) => {
    setDismissedIndices(prev => {
      const next = new Set(prev);
      next.add(index);
      return next;
    });
  };

  return (
    <group ref={groupRef} position={[0, yOffset, 0]}>
      {data.map((item, i) => {
        if (dismissedIndices.has(i)) return null;

        const angle = (i / data.length) * Math.PI * 2;
        const x = Math.sin(angle) * radius;
        const z = Math.cos(angle) * radius;

        return (
          <group key={`${item.label}-${i}`} position={[x, 0, z]} rotation={[0, angle, 0]}>
            <Html
              transform
              center
              scale={0.25}
              className="cursor-pointer pointer-events-auto"
            >
              <div className={`w-[768px] glass rounded-[4rem] p-12 border-4 border-primary/20 backdrop-blur-md shadow-2xl flex flex-col gap-6 items-center text-center relative group transition-all duration-500 hover:scale-[1.02] hover:border-primary/40 ${cardClassName}`}>
                {/* Close Button (The 'X') */}
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
                    handleDismiss(i);
                  }}
                  className="absolute top-10 right-10 w-16 h-16 rounded-full bg-primary/10 border-2 border-primary/20 flex items-center justify-center text-primary/40 group-hover:text-primary transition-all hover:bg-primary/20 active:scale-90"
                >
                  <X size={32} />
                </button>

                <span className="text-3xl uppercase tracking-[0.4em] text-muted-foreground font-bold">{item.title}</span>
                <span className="text-7xl font-extrabold text-foreground tracking-tight leading-tight">{item.label}</span>
                
                {item.tags && item.tags.length > 0 && (
                  <div className="flex flex-wrap gap-4 justify-center mt-6">
                    {item.tags.slice(0, 3).map((tag, idx) => (
                      <span key={idx} className="px-8 py-3 rounded-full bg-primary/5 border-2 border-primary/10 text-2xl font-mono text-primary/60 font-bold shadow-sm">
                        {tag}
                      </span>
                    ))}
                  </div>
                )}

                {/* Achievement Badge */}
                {item.type === 'achievement' && (
                  <div className="absolute -top-6 -right-6 px-10 py-4 bg-blue-500 rounded-full text-white text-2xl font-black uppercase tracking-widest shadow-xl rotate-12">
                    Verified
                  </div>
                )}
              </div>
            </Html>
          </group>
        );
      })}
    </group>
  );
};

export const Hero3DCarousel: React.FC<Hero3DCarouselProps> = ({ 
  imageSrc, 
  projects, 
  achievements = [] 
}) => {
  // Default placeholder data for achievements if none provided
  const displayAchievements = achievements.length > 0 ? achievements : [
    { title: "Recognition", label: "Global Innovation Award", tags: ["First Place", "2024"], type: 'achievement' },
    { title: "Achievement", label: "2M+ Active Users Reached", tags: ["Scalability", "Impact"], type: 'achievement' },
    { title: "Certification", label: "Full-Stack System Architect", tags: ["Advanced", "Core"], type: 'achievement' },
    { title: "Recognition", label: "Top Contributor @ Brainly", tags: ["Community", "Growth"], type: 'achievement' }
  ];

  return (
    <div className="w-full h-[450px] md:h-[550px] relative z-50">
      <Canvas camera={{ position: [0, 2, 18], fov: 35 }}>
        <ambientLight intensity={0.7} />
        <pointLight position={[10, 10, 10]} intensity={1.5} color="#3b82f6" />
        <pointLight position={[-10, -10, -10]} intensity={0.5} color="#60a5fa" />
        
        <group scale={0.85}>
          {/* Center Image */}
          <group position={[0, 0, 0]}>
            <Html transform center scale={0.25} className="pointer-events-none">
              <div className="relative w-[896px] h-[896px] rounded-full border-[12px] border-primary/30 overflow-hidden glass shadow-[0_0_150px_rgba(59,130,246,0.3)] p-4">
                <div className="w-full h-full rounded-full overflow-hidden relative group">
                  <img
                    src={imageSrc}
                    alt="Profile"
                    className="w-full h-full object-cover grayscale-[20%] group-hover:grayscale-0 transition-all duration-700"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-primary/20 to-transparent mix-blend-overlay" />
                </div>
              </div>
            </Html>
          </group>

          {/* Inner Ring: Projects */}
          <RotatingGroup 
            data={projects} 
            radius={4.2} 
            speed={0.15} 
            cardClassName="bg-background/40"
          />

          {/* Outer Ring: Achievements (Rotating in opposite direction) */}
          <RotatingGroup 
            data={displayAchievements} 
            radius={7.5} 
            speed={-0.12} 
            yOffset={0}
            cardClassName="bg-blue-500/5 border-blue-500/10"
          />
        </group>

        <OrbitControls
          enableZoom={false}
          enablePan={false}
          autoRotate={false} 
          minPolarAngle={Math.PI / 2.5}
          maxPolarAngle={Math.PI / 1.7}
        />
      </Canvas>
      
      {/* Decorative environment blur */}
      <div className="absolute inset-0 bg-blue-500/5 rounded-full blur-[150px] pointer-events-none -z-10" />
    </div>
  );
};
